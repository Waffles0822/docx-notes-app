...
In the lecture group only, a first-level bullet that has bullets nested under it is a compact parent label, not a sentence. Write it as a short noun phrase naming the topic its nested bullets explain, for example Controlled Act and Delegation, Enforcement Mechanisms, or Sources of Air Pollution. Never use generic parent labels such as Definition and Purpose, Overview, Key Points, Important Information, or Supporting Details. Use Title Case, capitalizing major words but not articles, conjunctions, or prepositions unless they are the first word. Never end it with a period, never write it in sentence format, and never build it around a finite verb. Do not use bold markup for any bullet. Keep it to roughly two to six words and push every fact, figure, and qualifier down into the bullets nested beneath it. A first-level bullet with nothing nested under it is not a heading and must be one complete sentence.
