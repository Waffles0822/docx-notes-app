...
// Preserve the hierarchy generated by the model. The referenced note structure lets
// standalone first-level facts remain sentences and uses short parent labels only
// when they genuinely organize child details. The exporter must not invent extra
// title-and-child pairs because doing so distorts that structure.
function normalizeBulletLayout(bullets: Bullet[]): Bullet[] {
  return bullets.map((bullet) => {
    const conciseText = condenseBulletWording(bullet.text)
    return { text: conciseText, children: normalizeBulletLayout(bullet.children) }
  })
}
