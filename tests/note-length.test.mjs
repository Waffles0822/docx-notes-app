...
function harness() {
  ...
  return {
    ...
    poll: (jobs, html = true) => route.POST({ json: async () => ({ jobs, returnNotesHtml: html }) }),
  }
}

test("21-page request creates 21 full-page writing tasks and rejects short drafts", async () => {
  const h = harness()
  ...
  for (const html of [true, false]) {
    ...
    assert.match((await response.json()).error, /40 words against a 300-word target/)
  }
  assert.equal(h.submissions.length, 63)
...
test("truncated replacements cannot export even when long enough", async () => {
  const h = harness()
  const jobs = [{ id: "resp_cut", targetWords: 1000, expanded: true, expansionAttempts: 2 }]
...
test("10 pages retain 3000 words and nine page boundaries in both exports", async () => {
  const h = harness()
  const jobs = await h.ai.startBackgroundNotes(Array(9000).fill("source").join(" "), 10)
  assert.equal(jobs.length, 10)
  assert.ok(jobs.every((job, i) => job.targetWords === 300 && job.pageNumber === i + 1))
  jobs.forEach((job, i) => h.statuses.set(job.id, { notes: notes(300).replace("Topic", `Topic${i + 1}`) }))
  const result = await (await h.poll(jobs)).json()
  ...
  assert.equal((xml.match(/<w:pageBreakBefore(?:\s[^>]*)?\/>/g) || []).filter(tag => !tag.includes('w:val="false"')).length, 9)
  ...
  assert.equal(requests.filter(request => request.updateParagraphStyle?.paragraphStyle?.pageBreakBefore === true).length, 9)
...
test("258 words triggers correction of one page, not another five-page summary", async () => {
  const h = harness()
  let jobs = await h.ai.startBackgroundNotes(Array(9000).fill("source").join(" "), 10)
  jobs.forEach((job, i) => h.statuses.set(job.id, { notes: notes(i === 0 ? 258 : 300) }))
  const correction = await (await h.poll(jobs)).json()
  assert.equal(correction.status, "processing")
  assert.equal(h.submissions.length, 11)
  assert.match(h.submissions[10].input, /Write 300 to 324 visible words/)
  jobs = correction.jobs
  h.statuses.set(jobs[0].id, { notes: notes(300) })
  assert.equal((await (await h.poll(jobs)).json()).status, "completed")
})
...
test("all 80 supported pages survive status validation", async () => {
  ...
  jobs.forEach((job, index) => h.statuses.set(job.id, { notes: notes(300).replace("Topic", `Topic${index}`) }))
  const result = await (await h.poll(jobs)).json()
  assert.equal(result.status, "completed")
